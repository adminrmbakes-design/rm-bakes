...
class Order(db.Model):
    ...
    # =====================================
    # PAYMENT METHOD
    # (payment_status lives in the PAYMENT INFORMATION
    # block further down, alongside payment_verified etc.)
    # =====================================
    ...
    # =====================================
    # CONVERSION METADATA
    # (production_started_at lives in the TIMESTAMPS
    # block further down, alongside created_at etc.)
    # =====================================
...
# =========================================
# ANALYTICS RESET  (FIX #4)
# =========================================
# Powers the "Reset Analytics" button on the admin analytics
# dashboard. We deliberately do NOT delete real Order / CustomOrder
# rows here — those are genuine business + customer records (order
# history, invoices, "My Orders", reorder, etc.) and destroying them
# would break the app for customers and wipe real transaction data.
#
# Instead, every "reset" simply inserts a new row with the current
# timestamp. The analytics dashboard then only counts orders placed
# AFTER the most recent reset_at, so the dashboard "starts fresh"
# exactly like the admin asked — without touching real records.

class AnalyticsReset(db.Model):

    __tablename__ = "analytics_resets"
...
class ProductReview(db.Model):
    ...
    # order_id/order_number are nullable — "Overall Experience" reviews
    # can now be left directly (Leave a Review), with no order attached.
    order_id = db.Column(
