...
def analytics_dashboard():
    ...
    # IST calendar date — a Chennai admin's "today" doesn't line up with
    # datetime.utcnow().date() (UTC midnight isn't IST midnight), so both
    # the reference point and every order compared against it need the
    # same IST-based date, or "today's revenue" can silently miss/include
    # the wrong orders near the day boundary.
    ...
    def _collected_since(start_date):
        return sum(
            o.grand_total or 0
            for o in active_orders
            if _is_collected(o) and o.ordered_at and to_ist(o.ordered_at).date() >= start_date
        )
    ...
    for days_back in range(6, -1, -1):
        ...
        day_orders = [
            o for o in active_orders
            if o.ordered_at and to_ist(o.ordered_at).date() == target_date
        ]
