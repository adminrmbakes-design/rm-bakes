...
# Only affects the currently logged-in admin's own account.
# =========================================
...
def admin_change_password():
    ...
    admin_user = User.query.get_or_404(session.get("admin_id"))
    ...
    if not check_password_hash(admin_user.password, old_password):
        ...
        return redirect(url_for("admin.admin_password_page"))
    ...
    # Session is left untouched on purpose — the admin stays logged in.
...
@admin_required
@feature_gate("cafe_notifications")
...
def admin_reviews():
    ...
    # =================================
    # FIX #5: RATING / VISIBILITY FILTER
    # (this was previously accepted from the URL but
    # never actually applied to the `reviews` list —
    # the tabs pointed at it were also using the wrong
    # query-param name, see admin_reviews.html)
    # =================================

    if review_filter == "hidden":
        ...
        reviews = [
            review
            for review in reviews
            if review.rating == int(review_filter)
        ]
    ...
    for review in reviews:
        ...
        # FIX #6: "Overall Experience" is a whole-order review, not
        # tied to any specific product — it was previously missing
        # from this exclusion list, so it kept winning "Highest Rated"
        # and "Most Reviewed" simply because almost every order gets
        # one. Excluded here alongside the other non-product sentinels.
