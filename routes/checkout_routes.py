...
from utils.order_email_utils import (
...
# ============================================================
# HELPER — FETCH PAYMENT MODE FROM RAZORPAY  (UPI/Card/etc.)
# ============================================================

# Razorpay returns the raw payment "method" (lowercase). We map
# it to a friendly label for storage in Order.payment_mode.
_RAZORPAY_METHOD_LABELS = {
    "upi":        "UPI",
    "card":       "Card",
...
def _fetch_razorpay_payment_mode(razorpay_payment_id: str) -> str | None:
    """
    Calls Razorpay's Fetch Payment API to find out HOW the customer
    paid (UPI / Card / Net Banking / EMI / Wallet ...) and returns a
    friendly label for storage in Order.payment_mode.

    Never raises — payment_mode is a nice-to-have, not something
    that should ever block order creation if Razorpay is slow or
    the lookup fails for any reason.
    """
    try:
        client  = _razorpay_client()
        payment = client.payment.fetch(razorpay_payment_id)
        raw_method = (payment.get("method") or "").strip().lower()
        if not raw_method:
            return None
        return _RAZORPAY_METHOD_LABELS.get(raw_method, raw_method.replace("_", " ").title())
    except Exception:
        logger.exception(
            "Could not fetch payment method from Razorpay for payment %s",
            razorpay_payment_id
        )
        return None
...
def create_payment():
    ...
    try:
        ...
        # BUGFIX: preferred_payment_method is stored as
        # "Cash On Delivery" (see profile.html / profile_routes.py
        # PAYMENT_METHODS), never as "COD". The old check below
        # compared against "COD" and therefore never matched, so
        # every COD order silently fell through to the Razorpay
        # (Path B) branch below. Comparing against the real stored
        # value fixes COD orders being placed directly.
        # ==================================================
        if payment_method == "Cash On Delivery":
            order = _create_order_record(
                ...
                payment_mode     = None,  # no online payment mode for COD
            )
        ...
        # ==================================================
        if not is_feature_live("payments"):
            return jsonify({
                "success": False,
                "message": "Temporary server error. Couldn't prepare payment. Please try again later."
            })
...
def verify_payment():
    ...
    try:
        ...
        # ── step 4.5: fetch payment mode (UPI/Card/Net Banking/EMI) from Razorpay ──
        payment_mode = _fetch_razorpay_payment_mode(rzp_payment_id)
